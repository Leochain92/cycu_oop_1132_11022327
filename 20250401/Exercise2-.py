...
def get_stop_info(stop_link: str):
    """下載並儲存指定站點的 HTML 頁面到 bus_stops 資料夾"""
    ...
    try:
        ...
        # 儲存 HTML 檔案到 bus_stops 資料夾
        with open(f"bus_stops/bus_stop_{stop_id}.html", "w", encoding="utf-8") as file:
            file.write(response.text)

        print(f"✅ 下載完成：bus_stops/bus_stop_{stop_id}.html")
...
def download_all_stops(df: pd.DataFrame):
    """多線程下載所有站點的 HTML 頁面到 bus_stops 資料夾"""
    os.makedirs("bus_stops", exist_ok=True)
